...
<div class="flex h-screen overflow-hidden">
    ...
    <aside :class="sidebarOpen ? 'w-64' : 'w-0 lg:w-16'"
        ...
        {{-- Lab Info --}}
        @php $__lab = Auth::user()->laboratory; @endphp
        <div x-show="sidebarOpen" x-cloak class="px-4 py-4 border-b border-teal-700">
            <div class="flex items-center gap-3">
                <img src="{{ $__lab && $__lab->profile_image ? asset('storage/'.$__lab->profile_image) : asset('images/default-lab.png') }}"
                ...
                <div class="overflow-hidden">
                    <p class="text-white font-semibold text-sm truncate">{{ $__lab->name ?? 'My Laboratory' }}</p>
                    <div class="flex items-center gap-1 mt-0.5">
                        @if($__lab && $__lab->status === 'approved')
        ...
        <nav class="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
            ...
            <a href="{{ route('laboratory.orders.index') }}"
               class="sidebar-link {{ request()->routeIs('laboratory.orders.*') ? 'active' : '' }}">
                ...
                @php $__pending = \App\Models\LabOrder::where('laboratory_id', $__lab->id ?? 0)->where('status','pending')->count(); @endphp
                @if($__pending > 0)
                <span x-show="sidebarOpen" x-cloak class="ml-auto bg-yellow-400 text-teal-900 text-xs font-bold px-2 py-0.5 rounded-full">{{ $__pending }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('laboratory.chat.index') }}"
               class="sidebar-link {{ request()->routeIs('laboratory.chat.*') ? 'active' : '' }}">
                ...
                @php $__unreadChat = \Illuminate\Support\Facades\DB::table('chat_messages')->where('receiver_id', Auth::id())->where('is_read', false)->count(); @endphp
                @if($__unreadChat > 0)
                <span x-show="sidebarOpen" x-cloak class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $__unreadChat }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('laboratory.notifications.index') }}"
               class="sidebar-link {{ request()->routeIs('laboratory.notifications.*') ? 'active' : '' }}">
                ...
                @php $__unread = \Illuminate\Support\Facades\DB::table('notifications')->where('notifiable_id', Auth::id())->where('is_read', false)->count(); @endphp
                @if($__unread > 0)
                <span x-show="sidebarOpen" x-cloak class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $__unread }}</span>
                @endif
            </a>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="bg-white border-b border-gray-200 px-6 h-16 flex items-center justify-between flex-shrink-0 shadow-sm z-30">
            ...
            <div class="flex items-center gap-3">
                ...
                {{-- Notification Bell --}}
                @php $__notifCount = \Illuminate\Support\Facades\DB::table('notifications')->where('notifiable_id', Auth::id())->where('is_read', false)->count(); @endphp
                <a href="{{ route('laboratory.notifications.index') }}"
                   class="relative p-2 text-gray-500 hover:text-teal-700 hover:bg-teal-50 rounded-lg transition">
                    <i class="fas fa-bell text-lg"></i>
                    @if($__notifCount > 0)
                    <span class="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{{ $__notifCount > 9 ? '9+' : $__notifCount }}</span>
                    @endif
                </a>

                {{-- Chat --}}
                @php $__chatCount = \Illuminate\Support\Facades\DB::table('chat_messages')->where('receiver_id', Auth::id())->where('is_read', false)->count(); @endphp
                <a href="{{ route('laboratory.chat.index') }}"
                   class="relative p-2 text-gray-500 hover:text-teal-700 hover:bg-teal-50 rounded-lg transition">
                    <i class="fas fa-comment-dots text-lg"></i>
                    @if($__chatCount > 0)
                    <span class="absolute -top-0.5 -right-0.5 bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{{ $__chatCount }}</span>
                    @endif
                </a>
                ...
                <div x-data="{ open: false }" class="relative">
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak
                        ...
                        <div class="px-4 py-2 border-b border-gray-100 mb-1">
                            <p class="font-semibold text-gray-900 text-sm truncate">{{ Auth::user()->laboratory->name ?? 'Laboratory' }}</p>
                            <p class="text-xs text-gray-400 truncate">{{ Auth::user()->email }}</p>
                        </div>
                        <a href="{{ route('laboratory.profile.index') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-teal-50 hover:text-teal-700 transition">
                            <i class="fas fa-building w-4"></i> Lab Profile
                        </a>
                        <a href="{{ route('laboratory.settings') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-teal-50 hover:text-teal-700 transition">
