...
@section('title', 'Laboratories Management')
@section('page-title', 'Laboratories Management')
...
@section('content')
    ...
    <div class="dashboard-card mb-4">
        <div class="card-header">
            <h6><i class="fas fa-flask me-2"></i>All Laboratories</h6>
        ...
        <div class="card-body">
            <!-- Filters Form -->
            <form action="{{ route('admin.laboratories.index') }}" method="GET" class="row g-3 mb-3">
                <div class="col-md-4">
            ...
            <div class="row g-3 mb-3">
                ...
                <div class="col-md-3">
                    <div class="stat-summary stat-summary-success">
                        ...
                        <div class="stat-details">
                            <h6>Approved</h6>
                            <h4>{{ \App\Models\Laboratory::where('status', 'approved')->count() }}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="stat-summary stat-summary-warning">
                        ...
                        <div class="stat-details">
                            <h6>Pending</h6>
                            <h4>{{ \App\Models\Laboratory::where('status', 'pending')->count() }}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="stat-summary stat-summary-danger">
                        ...
                        <div class="stat-details">
                            <h6>Suspended</h6>
                            <h4>{{ \App\Models\Laboratory::where('status', 'suspended')->count() }}</h4>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <small class="text-muted">{{ $lab->email }}</small>
                            </td>
                            <td>
                                <span class="badge bg-info">{{ $lab->registration_number }}</span>
                            </td>
                            <td>{{ $lab->city }}</td>
                            <td>
                            ...
                            <td>
                                ...
                                <small>({{ number_format($lab->rating ?? 0, 1) }})</small>
                            </td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm" role="group">
                                    <a href="{{ route('admin.laboratories.show', $lab->id) }}"
                                        class="btn btn-info" title="View" data-bs-toggle="tooltip">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('admin.laboratories.edit', $lab->id) }}"
                                        class="btn btn-warning" title="Edit" data-bs-toggle="tooltip">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    @if($lab->status == 'pending')
                                        <button onclick="approveLab({{ $lab->id }}, '{{ addslashes($lab->name) }}')"
                                            class="btn btn-success" title="Approve" data-bs-toggle="tooltip">
                                            <i class="fas fa-check"></i>
                                        </button>
                                        <button onclick="rejectLab({{ $lab->id }}, '{{ addslashes($lab->name) }}')"
                                            class="btn btn-danger" title="Reject" data-bs-toggle="tooltip">
                                            <i class="fas fa-times"></i>
                                        </button>
                                    @elseif($lab->status == 'approved')
                                        <button onclick="suspendLab({{ $lab->id }}, '{{ addslashes($lab->name) }}')"
                                            class="btn btn-secondary" title="Suspend" data-bs-toggle="tooltip">
                                            <i class="fas fa-ban"></i>
                                        </button>
                                    @elseif($lab->status == 'suspended' || $lab->status == 'rejected')
                                        <button onclick="activateLab({{ $lab->id }}, '{{ addslashes($lab->name) }}')"
                                            class="btn btn-success" title="Activate" data-bs-toggle="tooltip">
                                            <i class="fas fa-check-circle"></i>
                                        </button>
                                    @endif
                                    <button onclick="deleteLab({{ $lab->id }}, '{{ addslashes($lab->name) }}')"
            ...
            <div class="mt-4 d-flex justify-content-between align-items-center">
                <div class="text-muted">
                    Showing {{ $laboratories->firstItem() ?? 0 }} to {{ $laboratories->lastItem() ?? 0 }} of {{ $laboratories->total() }} entries
                </div>
                <div>
    ...
    <form id="deleteLabForm" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
    </form>
@endsection
...
<script>
    ...
    function approveLab(id, name) {
        Swal.fire({
            ...
            preConfirm: () => {
                ...
                return fetch(`/admin/laboratories/${id}/approve`, {
                    method: 'POST',
                    body: formData
                }).then(response => {
    ...
    function rejectLab(id, name) {
        Swal.fire({
            title: 'Reject Laboratory?',
            text: name,
            icon: 'warning',
            showCancelButton: true,
            ...
            preConfirm: () => {
                const formData = new FormData();
                formData.append('_token', document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
                return fetch(`/admin/laboratories/${id}/reject`, {
                    method: 'POST',
                    body: formData
                }).then(response => {
                    if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (result.isConfirmed) {
                Toast.fire({ icon: 'success', title: 'Laboratory Rejected!' }).then(() => location.reload());
            }
        });
    }

    function suspendLab(id, name) {
        Swal.fire({
            ...
            preConfirm: () => {
                const formData = new FormData();
                formData.append('_token', document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
                return fetch(`/admin/laboratories/${id}/suspend`, {
                    method: 'POST',
                    body: formData
                }).then(response => {
                    if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (result.isConfirmed) {
                Toast.fire({ icon: 'success', title: 'Laboratory Suspended!' }).then(() => location.reload());
            }
        });
    }

    function activateLab(id, name) {
        Swal.fire({
            ...
            preConfirm: () => {
                const formData = new FormData();
                formData.append('_token', document.querySelector('meta[name="csrf-token"]').getAttribute('content'));
                return fetch(`/admin/laboratories/${id}/activate`, {
                    method: 'POST',
                    body: formData
                }).then(response => {
                    if (!response.ok) throw new Error('Network response was not ok');
            ...
            if (result.isConfirmed) {
                Toast.fire({ icon: 'success', title: 'Laboratory Activated!' }).then(() => location.reload());
            }
        });
    }
    ...
    function deleteLab(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.action = `/admin/laboratories/${id}`;
                form.submit();
            }
        });
    }
