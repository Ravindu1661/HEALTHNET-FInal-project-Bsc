...
public function pay(Request $request, $appointmentId)
{
    ...
    $fullFee     = (float) ($appointment->consultation_fee ?? 0);
    $advanceFee  = round($fullFee * 0.5, 2);

    // ✅ Determine actual charge amount
    $chargeAmount = ($paymentType === 'advance') ? $advanceFee : $fullFee;

    try {
        \Stripe\Stripe::setApiKey(config('services.stripe.secret'));

        // ✅ Use chargeAmount (not always full fee)
        ...
        $paymentIntent = \Stripe\PaymentIntent::create([
            ...
            'description'         => 'HealthNet Appointment #' . $appointmentId .
                                     ($paymentType === 'advance' ? ' (Advance 50%)' : ' (Full)'),
            'metadata'            => [
                'appointment_id' => $appointmentId,
                'patient_id'     => $patient->id,
                'payment_type'   => $paymentType,   // ✅ metadata ට add
            ],
        ]);
        ...
        if ($paymentIntent->status === 'succeeded') {
            $this->markAppointmentPaid(
                ...
                $paymentType,    // ✅ pass payment type
                $chargeAmount    // ✅ pass actual charged amount
            );

            $msg = $paymentType === 'advance'
                ? '✅ Advance payment of Rs. ' . number_format($chargeAmount, 2) . ' successful! Balance due at appointment.'
                : '✅ Full payment successful! Your appointment is confirmed.';
        ...
        if ($paymentIntent->status === 'requires_action'
            ...
            session(['pending_payment_type'   => $paymentType]);    // ✅ session ට save
            session(['pending_charge_amount'  => $chargeAmount]);   // ✅ session ට save
...
public function paymentCallback(Request $request)
{
    ...
    $paymentType   = session('pending_payment_type', 'full');   // ✅
    $chargeAmount  = session('pending_charge_amount', null);    // ✅
    ...
    try {
        ...
        if ($paymentIntent->status === 'succeeded') {
            // ✅ chargeAmount — session ගෙ, නැතිනම් fee calculate
            $fullFee      = (float) ($appointment->consultation_fee ?? 0);
            $actualAmount = $chargeAmount ?? ($paymentType === 'advance'
                ? round($fullFee * 0.5, 2)
                : $fullFee);
            ...
            $msg = $paymentType === 'advance'
                ? '✅ Advance payment successful! Balance due at appointment.'
                : '✅ Full payment successful! Your appointment is confirmed.';
...
private function markAppointmentPaid(
    ...
    string $paymentType   = 'full',   // ✅ NEW param
    ?float $chargeAmount  = null      // ✅ NEW param
): void {
    DB::transaction(function () use (
        $appointment, $transactionId, $patientId,
        $cardholderName, $paymentType, $chargeAmount
    ) {
        $fullFee = (float) ($appointment->consultation_fee ?? 0);

        // ✅ Determine actual charged amount
        if ($chargeAmount === null) {
            $chargeAmount = ($paymentType === 'advance')
                ? round($fullFee * 0.5, 2)
                : $fullFee;
        }

        // ✅ Determine new payment_status + advance_payment
        if ($paymentType === 'advance') {
            $newPaymentStatus  = 'partial';
            $newAdvancePayment = $chargeAmount;
        } else {
            $newPaymentStatus  = 'paid';
            $newAdvancePayment = $fullFee; // full payment — advance = full fee
        }
        ...
        $payNotes = $cardholderName
            ? 'Cardholder: ' . $cardholderName
            : 'Online card payment via Stripe';

        if ($paymentType === 'advance') {
            $payNotes .= ' | Advance (50%) Payment — Balance Rs. ' .
                         number_format($fullFee - $chargeAmount, 2) . ' due at appointment';
        }
        ...
        DB::table('payments')->insert([
            ...
            'amount'         => $chargeAmount,       // ✅ actual charged amount
            'payment_method' => 'card',
        ...
        // ✅ Notification message — advance vs full
        $payTypeLabel = $paymentType === 'advance'
            ? 'Advance (50%) payment of Rs. ' . number_format($chargeAmount, 2) .
              ' (Balance Rs. ' . number_format($fullFee - $chargeAmount, 2) . ' due at appointment)'
            : 'Full payment of Rs. ' . number_format($chargeAmount, 2);
        ...
        try {
            DB::table('notifications')->insert([
                ...
                'message'         => $payTypeLabel .
                                ...
                                ($paymentType === 'advance'
                                    ? ' Appointment is pending doctor confirmation. Please pay balance at appointment.'
